...
# ======== إرسال الرسائل باستخدام requests =========
def send_sms(number):
...
async def send_task_function(bot, chat_id, user_id, numbers_list):
    ...
    for number in numbers_list:
        ...
        for i in range(count):
            if stop_flags.get(user_id):
                break
            status, response_text = send_sms(number)
            if status == 200:
                ...
                await bot.send_message(
                    chat_id=chat_id, text=f"⚠️ فشل إرسال الرسالة {i+1} إلى {number} - الحالة: {status}, الاستجابة: {response_text}"
                )
