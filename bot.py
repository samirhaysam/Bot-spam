...
URL1 = os.getenv("API_URL_1", "https://api.twistmena.com/music/Dlogin/sendCode")
URL2 = os.getenv("API_URL_2", "https://mab.etisalat.com.eg:11003/Saytar/rest/quickAccess/sendVerCodeQuickAccessV4")
...
async def send_sms_async(session, number, use_api_1=True):
    if use_api_1:
        ...
        try:
            async with session.post(URL1, json=payload, headers=headers) as response:
                ...
                logger.info(f"API الأول - الحالة: {response.status}, الاستجابة: {response_text}")
                return response.status, response_text, use_api_1
        except Exception as e:
            logger.error(f"خطأ في API الأول: {e}")
        ...
        try:
            async with session.post(URL2, data=payload, headers=HEADERS_API2) as response:
                response_text = await response.text()
                logger.info(f"API الثاني - الحالة: {response.status}, الاستجابة: {response_text}")
                return response.status, response_text, use_api_1
        except Exception as e:
            logger.error(f"خطأ في API الثاني: {e}")
            return None, str(e), use_api_1
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for number in numbers_list:
            ...
            for i in range(count):
                ...
                status, response_text, api_used = await send_sms_async(session, number, use_api_1)
